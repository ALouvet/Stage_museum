# -*- coding: utf-8 -*-
"""
Created on Mon Dec 10 09:52:13 2018
...
def generation_image(matrice, nom):
    ...
    plt.xlabel('probabilite de mort des graines')
    plt.ylabel('probabilte de germination')
    plt.title('probabilite de colonisation : '+str(col))
    plt.savefig(nom+'.png')
    
    #Creation d'un fichier .csv pour pouvoir reconstruire les images si necessaire
...
def fonction_test(c, g, d):
    ...
    for l in range(N_param): #Nombre de simulations realisees pour chaque jeu de parametres
        ...
        simulation_prm_csv(pop, t, 'tempgi')
        liste_data = lire_colonne_csv(lire_csv('tempgi'))
        ...
        for m in range(N_BGS):
            ...
            for i in range(N_it):
                ...
                if resultat[2] < AIC :
                    mod = 1
                    break #On s'arrete des que la banque de graines est mieux
        if mod == 1:
            test += 1
...
if __name__ == '__main__':
    ...
    col = float(raw_input('Entrer la probabilité de colonisation :'))
